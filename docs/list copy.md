...
线性表是最基本、最常用的一种线性数据结构。线性表被广泛应用于信息存储与管理、网络、通信等诸多领域。
本章将给出线性表的定义和抽象数据类型描述，讨论线性表的逻辑结构、存储结构及相关运算，并以一元整系数多项式的算术运算为实例介绍线性表的简单应用。

「线性表 list」是零个或若干个数据元素构成的线性序列，记为($a_0$,a~1~,...,a~n-1~)。
